...
static void tilevertical(Monitor *m);
...
void tilevertical(Monitor *m) {
	unsigned int w, r, e = m->gaps, mh, mx, tx;
	int i, n = 0;
	Client *c;
	...
	wl_list_for_each(c, &clients, link) {
		...
		if (i < m->nmaster) {
			...
			r = n - i;
			w = (m->w.width - tx - gappx*e - gappx*e * (r - 1)) / r;
			resize(c, (struct wlr_box){.x = m->w.x + tx, .y = m->w.y + mh,
				.width = w, .height = m->w.height - mh - gappx*e}, 0);
